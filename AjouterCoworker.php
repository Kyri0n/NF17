...
/* Connexion à la base de données */
	...
	$ajoutDomaine=$_POST["ajoutDomaine"];
	$supprimeDomaine=$_POST["supprimeDomaine"];
	...
	if($modifier=='True'){
		...
		if($ajoutDomaine){
			$vSql="INSERT INTO assoc_coworkerdomaine values ('$ajoutDomaine',$id)";
			$vQuery=pg_query($vConn, $vSql);
			$result=pg_fetch_row($vQuery);
		}
		if($supprimeDomaine){
			$vSql="DELETE FROM assoc_coworkerdomaine WHERE info_domaine = $supprimeDomaine AND Coworker=$id";
			$vQuery=pg_query($vConn, $vSql);
			$result=pg_fetch_row($vQuery);
		}
		pg_close($vConn);
